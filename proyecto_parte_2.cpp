...
int descartarMunicion(Personaje* pj) {
    if (!pj->arsenal) {
        cout << "   [SIN MUNICION] " << pj->nombre << " no tiene armas, ataca solo con su fuerza base.\n";
        return 0;
    }
    ...
    delete n;
    if (!pj->arsenal)
        cout << "   [ARSENAL AGOTADO] " << pj->nombre << " gasto su ultima arma y se quedo sin municion.\n";
    return dmg;
}
...
int contarHeroesBando(NodoBtree* nodo, int bando) {
    if (!nodo) return 0;
    int c = 0;
    for (int i = 0; i < nodo->cant_pj; i++)
        if (nodo->Personajes[i]->ID_Bando == bando && nodo->Personajes[i]->esHeroe) c++;
    if (!nodo->hoja)
        for (int i = 0; i <= nodo->cant_pj; i++)
            c += contarHeroesBando(nodo->hijos[i], bando);
    return c;
}
...
Heroes elegirHeroe(NodoHeroe* listaHeroes) {
    cout << "========================================\n";
    cout << "   SELECCION DE HEROE\n";
    cout << "========================================\n";

    if (!listaHeroes) {
        cout << "   No hay heroes cargados en heroes.txt.\n";
        return {0, "Desconocido", 0, 0, 0};
    }

    for (NodoHeroe* a = listaHeroes; a; a = a->siguiente) {
        cout << "  " << a->dato.id << ". " << a->dato.nombre
             << " | HP:" << a->dato.salud
             << " DMG:" << a->dato.damage
             << " RAP:" << a->dato.rapidez << "\n";
    }

    Heroes hElegido = {0, "Desconocido", 0, 0, 0};
    bool encontrado = false;
    do {
        int idElegido = leerEnteroValidado(">> ", 1, 1000);
        for (NodoHeroe* a = listaHeroes; a; a = a->siguiente)
            if (a->dato.id == idElegido) { hElegido = a->dato; encontrado = true; break; }
        if (!encontrado) cout << "   Ese ID de heroe no existe, intenta de nuevo.\n";
    } while (!encontrado);
...
Escudo crearEscudoCustom() {
    ...
    cout << "--- CREACION DE ESCUDO PERSONALIZADO ---";
    cin.ignore();
            ...
            try {
                ...
                if (id_arma >= 1 && id_arma <= 5) {
                    if (id_arma == 5 && pj->Clase_ID != 1) {
                        ...
                        if (!actual) cout << "   Arma no encontrada en armas.txt, elige otra.\n";
                        else valido = true;
                    }
...
void Insertar_PJ(int bando, NodoEspecie* listaEspecies, NodoClase* listaClases, NodoArma* listaArmas, NodoEscudo* listaEscudos, NodoHeroe* listaHeroes) {
    limpiarPantalla();
    cout << "========================================\n";
    ...
    int heroesActuales = contarHeroesBando(Raiz_Btree, bando);

    cout << "\nTipo de personaje a insertar:\n";
    cout << "  1. Heroe (predefinido: sin clase ni especie, maximo 2 vivos por bando)\n";
    cout << "  2. Personaje normal (elige clase y especie)\n";
    if (heroesActuales >= 2)
        cout << "  [!] El Bando " << bando << " ya tiene " << heroesActuales
             << " heroe(s) activo(s). No puedes insertar otro hasta que uno caiga.\n";

    int tipoPersonaje = leerEnteroValidado(">> ", 1, 2);

    if (tipoPersonaje == 1 && heroesActuales >= 2) {
        cout << "   Limite de heroes alcanzado. Se creara un personaje normal en su lugar.\n";
        tipoPersonaje = 2;
    }

    if (tipoPersonaje == 1) {
        Heroes hElegido = elegirHeroe(listaHeroes);

        nuevo->esHeroe  = true;
        nuevo->Clase_ID = 0;
        nuevo->nombre   = hElegido.nombre;

        nuevo->Personaje_comb.nombreCompleto = hElegido.nombre;
        nuevo->Personaje_comb.saludTotal     = hElegido.salud;
        nuevo->Personaje_comb.damageTotal    = hElegido.damage;
        nuevo->Personaje_comb.rapidezTotal   = hElegido.rapidez;

        cout << "\n   [HEROE] " << nuevo->nombre << " se une al combate (stats predeterminadas).\n";
        cout << "   [STATS FINALES] HP:" << nuevo->Personaje_comb.saludTotal
...
int turnoInsercion(int bando, int turnoNum, int inyecciones, NodoEspecie* listaEspecies, NodoClase* listaClases, NodoArma* listaArmas, NodoEscudo* listaEscudos, NodoHeroe* listaHeroes) {
    limpiarPantalla();
    cout << "========================================\n";
    ...
    for (int i = 0; i < dado; i++) {
        ...
        cout << "   Insertando personaje " << (i + 1) << " de " << dado
             << " (Bando " << bando << ")\n";
        cout << "----------------------------------------\n";
        Insertar_PJ(bando, listaEspecies, listaClases, listaArmas, listaEscudos, listaHeroes);
    }
    return dado;
}


void Menu_Juego(NodoBtree*& raiz, NodoEspecie* listaEspecies, NodoClase* listaClases, NodoArma* listaArmas, NodoEscudo* listaEscudos, NodoHeroe* listaHeroes) {
    limpiarPantalla();
    cout << "========================================\n";
    ...
    while (jugando) {
        ...
        inyeccionesB1 += turnoInsercion(bandoInicia, turnoNum, inyeccionesB1, listaEspecies, listaClases, listaArmas, listaEscudos, listaHeroes);
        inyeccionesB2 += turnoInsercion(bandoEspera,  turnoNum, inyeccionesB2, listaEspecies, listaClases, listaArmas, listaEscudos, listaHeroes);
...
void mostrarTutorial() {
    limpiarPantalla();
    cout << "============================================\n";
    cout << "      TUTORIAL - COMO SE JUEGA YGGDRASIL\n";
    cout << "============================================\n";
    cout << " El juego es 1 contra 1 (Bando 1: Omegas vs\n";
    cout << " Bando 2: Neones). Todos los personajes viven\n";
    cout << " dentro de un Arbol B-4, y cada turno se repiten\n";
    cout << " las mismas 3 fases hasta que alguien gane.\n";
    cout << "============================================\n";
    pausar();

    limpiarPantalla();
    cout << "============================================\n";
    cout << "   FASE 1: INSERCION DE PERSONAJES\n";
    cout << "============================================\n";
    cout << " Al inicio de cada turno cada jugador tira un\n";
    cout << " dado (1-3) que indica cuantos personajes debe\n";
    cout << " insertar ese turno en el Arbol B.\n\n";
    cout << " Por cada personaje se elige:\n";
    cout << "   1. Heroe: predefinido (stats fijas de\n";
    cout << "      heroes.txt), SIN elegir clase ni especie.\n";
    cout << "      Maximo 2 heroes vivos por bando. Si uno\n";
    cout << "      de tus heroes muere, puedes invocar otro\n";
    cout << "      para volver a tener 2.\n";
    cout << "   2. Personaje normal: eliges una Clase\n";
    cout << "      (1-5) y una Especie (existente o\n";
    cout << "      personalizada); sus stats se suman.\n\n";
    cout << " Luego, a TODOS (heroe o normal) se les asigna\n";
    cout << " al azar una cantidad de escudos y de armas\n";
    cout << " (podras elegir cuales, dentro del dado tirado).\n";
    cout << "============================================\n";
    pausar();

    limpiarPantalla();
    cout << "============================================\n";
    cout << "   FASE 2: HABILIDADES\n";
    cout << "============================================\n";
    cout << " Cada jugador puede usar 1 habilidad por turno\n";
    cout << " con su personaje activo (el primero de su\n";
    cout << " bando que aparezca en el Arbol):\n\n";
    cout << "   1. Control Mental: solo el Hacker de Combate\n";
    cout << "      puede usarla, cambia de bando a un\n";
    cout << "      personaje enemigo por su ID.\n";
    cout << "   2. Sanacion: cualquier personaje sacrifica\n";
    cout << "      30 HP propios para curar al aliado con\n";
    cout << "      menos HP.\n";
    cout << "   3. Robo de Escudos: roba todos los escudos\n";
    cout << "      de los enemigos presentes en tu mismo\n";
    cout << "      nodo del Arbol.\n\n";
    cout << " Tambien puedes 'Pasar' o 'Rendirte' (0), lo\n";
    cout << " cual termina la partida de inmediato.\n";
    cout << "============================================\n";
    pausar();

    limpiarPantalla();
    cout << "============================================\n";
    cout << "   FASE 3: COMBATE AUTOMATICO\n";
    cout << "============================================\n";
    cout << " Cuando dos personajes de bandos distintos\n";
    cout << " comparten un mismo nodo del Arbol, combaten\n";
    cout << " automaticamente:\n\n";
    cout << "   - Ataca primero quien tenga mas Rapidez.\n";
    cout << "   - El atacante dispara su siguiente arma en\n";
    cout << "     cola (si le queda municion) y suma su\n";
    cout << "     Danio de arma + Danio de personaje.\n";
    cout << "   - Ese danio primero lo absorben los escudos\n";
    cout << "     del defensor (de a uno, hasta agotarse) y\n";
    cout << "     el resto resta de su HP.\n";
    cout << "   - Si el defensor sobrevive, contraataca con\n";
    cout << "     su propia arma.\n";
    cout << "   - Si un personaje llega a HP <= 0, muere: el\n";
    cout << "     ganador se queda con todo su equipo (armas\n";
    cout << "     y escudos) y se elimina del Arbol.\n";
    cout << "============================================\n";
    pausar();

    limpiarPantalla();
    cout << "============================================\n";
    cout << "   CONDICIONES DE VICTORIA\n";
    cout << "============================================\n";
    cout << "   1. Aniquilacion: eliminas a todos los\n";
    cout << "      personajes del bando enemigo.\n";
    cout << "   2. Inyecciones: tu bando acumula 50 o mas\n";
    cout << "      personajes insertados en total.\n";
    cout << "   3. Dominio de la raiz: tu bando es el UNICO\n";
    cout << "      presente en el nodo raiz del Arbol\n";
    cout << "      durante 3 turnos seguidos.\n";
    cout << "   4. Rendicion: si un jugador se rinde, el\n";
    cout << "      otro bando gana de inmediato.\n";
    cout << "============================================\n";
    cout << " Consejo: revisa los menus 3 y 4 del menu\n";
    cout << " principal para ver en detalle las clases,\n";
    cout << " especies, armas y escudos disponibles.\n";
    cout << "============================================\n";
    pausar();
}
...
void Menu_Bienvenida(NodoEspecie* listaEspecies, NodoClase* listaClases, NodoArma* listaArmas, NodoEscudo* listaEscudos, NodoHeroe* listaHeroes) {
    char res = 0;
    while (res != '7') {
        limpiarPantalla();
        cout << "========================================\n";
        ...
        switch (res) {
            case '1':
                Menu_Juego(Raiz_Btree, listaEspecies, listaClases, listaArmas, listaEscudos, listaHeroes);
                break;
            case '2':
                Sinopsis_Historia();
                break;
...
int main() {
    ...
    if (!listaHeroes)   cout << " heroes.txt no cargado.\n";

    Menu_Bienvenida(listaEspecies, listaClases, listaArmas, listaEscudos, listaHeroes);
